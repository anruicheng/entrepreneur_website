package an.easycodedemo.dao;

import an.easycodedemo.entity.Competition;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (Competition)表数据库访问层
 *
 * @author makejava
 * @since 2020-03-31 21:28:52
 */
public interface CompetitionDao {

    /**
     * 通过ID查询单条数据
     *
     * @param competitionId 主键
     * @return 实例对象
     */
    Competition queryById(Integer competitionId);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<Competition> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param competition 实例对象
     * @return 对象列表
     */
    List<Competition> queryAll(Competition competition);

    /**
     * 新增数据
     *
     * @param competition 实例对象
     * @return 影响行数
     */
    int insert(Competition competition);

    /**
     * 修改数据
     *
     * @param competition 实例对象
     * @return 影响行数
     */
    int update(Competition competition);

    /**
     * 通过主键删除数据
     *
     * @param competitionId 主键
     * @return 影响行数
     */
    int deleteById(Integer competitionId);

}