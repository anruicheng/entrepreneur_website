package an.easycodedemo.dao;

import an.easycodedemo.entity.PostMessage;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (PostMessage)表数据库访问层
 *
 * @author makejava
 * @since 2020-03-30 09:28:12
 */
public interface PostMessageDao {

    /**
     * 通过ID查询单条数据
     *
     * @param messageId 主键
     * @return 实例对象
     */
    PostMessage queryById(Integer messageId);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<PostMessage> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);
    List<PostMessage> searchByContent(String key);

    /**
     * 通过实体作为筛选条件查询
     *
     * @param postMessage 实例对象
     * @return 对象列表
     */
    List<PostMessage> queryAll(PostMessage postMessage);

    /**
     * 新增数据
     *
     * @param postMessage 实例对象
     * @return 影响行数
     */
    int insert(PostMessage postMessage);

    /**
     * 修改数据
     *
     * @param postMessage 实例对象
     * @return 影响行数
     */
    int update(PostMessage postMessage);

    /**
     * 通过主键删除数据
     *
     * @param messageId 主键
     * @return 影响行数
     */
    int deleteById(Integer messageId);

}